...
%token FUNC DO RETURN ARROW PRINT READ
%token ASSIGN IS IF THEN ELSE END
...
dec_list
	: declaration 											{ $$ = new DeclarationStatementList((DeclarationStatement)$1, new TextSpan($1.Span)); }
...
type
	: INTEGER												{ $$ = new IntegerType(new TextSpan($1.Span)); }
...
value		
	: BOOLEAN_VALUE											{ $$ = new BooleanValue((bool) ((TokenValue)$1)[0], new TextSpan($1.Span)); }
...
cond
	: IF math_expr THEN expr ELSE expr END					{ $$ = new Conditional((FExpression)$2, (FExpression)$4, (FExpression)$6, new TextSpan($1.Span, $7.Span)); }
	;

func_def
	: FUNC LROUND opt_params RROUND opt_type func_body		{ $$ = new FunctionDefinition((ParameterList)$3, (FType)$5, (StatementList)$6, new TextSpan($1.Span, $6.Span)); }
	;
...
param		
	: identifier COLON type									{ $$ = new Parameter((Identifier)$1, (FType)$3, new TextSpan($1.Span, $3.Span)); }
	;
...
func_call
	: secondary LROUND opt_exprs RROUND						{ $$ = new FunctionCall((FSecondary)$1, (ExpressionList)$3, new TextSpan($1.Span, $4.Span)); }
	;
...
if_stm
	: IF math_expr THEN stm_list e_if_list opt_else			{ $$ = new IfStatement((FExpression)$2, (StatementList)$4, (ElseIfList)$5, (StatementList)$6, new TextSpan($1.Span, $6.Span)); } //even if last is optional, it's always at least END so it has a TextSpan value 
...
e_if_list
	: ELSE IF math_expr THEN stm_list						{ $$ = new ElseIfList(new ElseIfStatement((FExpression) $3, (StatementList) $5, new TextSpan($1.Span, $5.Span)), new TextSpan($1.Span, $5.Span)); }
...
opt_else
	: END													{ $$ = new StatementList(new TextSpan($1.Span)); }
	| ELSE nif_stmlist END									{ $2.Span = new TextSpan($1.Span, $3.Span); $$ = $2; }
	;

nif_stmlist
	: nif_stm												{ $$ = new StatementList((FStatement)$1, $1.Span); }
...
return_stm
	: RETURN SEMICOLON										{ $$ = new ReturnStatement(new TextSpan($1.Span)); }		/* possible fix recognize new line before END token */
	| RETURN expr SEMICOLON									{ $$ = new ReturnStatement((FExpression)$2, new TextSpan($1.Span)); }
	;

break_stm
	: BREAK SEMICOLON										{ $$ = new BreakStatement(new TextSpan($1.Span)); }
	;

cont_stm
	: CONTINUE SEMICOLON									{ $$ = new ContinueStatement(new TextSpan($1.Span)); }
	;
...
array_def
	: LSQUARE opt_exprs RSQUARE 							{ $$ = new ArrayDefinition((ExpressionList)$2, new TextSpan($1.Span, $3.Span)); }
	;

map_def
	: LCURLY pair_list RCURLY 								{ $$ = new MapDefinition((ExpressionPairList)$2, new TextSpan($1.Span, $3.Span)); }
	;

pair_list
	: /* empty */											{ $$ = new ExpressionPairList(); }
...
pair
	: expr COLON expr										{ $$ = new ExpressionPair((FExpression)$1, (FExpression)$3, new TextSpan($1.Span, $3.Span)); }
	;

tuple_def
	: LROUND tuple_elist RROUND								{ $$ = new TupleDefinition((TupleElementList)$2, new TextSpan($1.Span, $3.Span)); }
	;
...
tuple_elem
	: identifier IS expr									{ $$ = new TupleElement((Identifier)$1, (FExpression)$3, new TextSpan($1.Span, $3.Span)); }
	| expr													{ $$ = new TupleElement(null, (FExpression)$1, new TextSpan($1.Span)); }
	;

indexer
	: LSQUARE expr RSQUARE									{ $$ = new SquaresIndexer((FExpression)$2, new TextSpan($1.Span, $3.Span)); }
...
func_type
	: FUNC LROUND type_list RROUND COLON type 				{ $$ = new FunctionType((TypeList)$3, (FType)$6, new TextSpan($1.Span, $6.Span)); }
	;
...
array_type
	: LSQUARE type RSQUARE 									{ $$ = new ArrayType((FType)$2, new TextSpan($1.Span, $3.Span)); }
	;

tuple_type
	: LROUND type_list RROUND								{ $$ = new TupleType((TypeList)$2, new TextSpan($1.Span, $3.Span)); }
	;

map_type
	: LCURLY type COLON type RCURLY 						{ $$ = new MapType((FType)$2, (FType)$4, new TextSpan($1.Span, $5.Span)); }
	;
